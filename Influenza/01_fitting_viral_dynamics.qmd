...
```{r load_data}
Dmax =  14.5 # Maximu follow-up duration to be included: 5.5 or 8 
...
model_settings$Nwarmup = 100
model_settings$Niter = 500
model_settings$Nthin = 4
```
...
```{r}
trace <- traceplot(out, pars = c("sigmasq_u", "coef", "intercept"))
trace
```
...
```{r}
model_selects <- 1
ff = "Rout/model_fits_type_A_14d_job_2.RData"
