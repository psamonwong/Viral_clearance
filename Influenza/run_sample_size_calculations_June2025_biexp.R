...
# use the bi-exponential model fits 
load('Rout/model_fits_job_10.RData')
...
save(sim_settings, file = 'Rout/sim_settings_Jun25_biexponential.RData')
...
# simulate data
set.seed(i)
sim_vl = sim_individuals_biexp(thetas = thetas,
                         t_design = t_design_i,
                         N = length(Trt_vector),
                         trt_effects = trt_effects,
                         Trt_arm = Trt_vector,
                         LOD = my_LOD,
                         f_sim_biexp  = f_sim_biexp)
...
f_name = paste0('sims_out_June2025_biexp/sim_out',i,'.csv')
print(f_name)
