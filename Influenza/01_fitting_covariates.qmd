...
use_threshold = T
Type <- c("A", "B", "All")
all_mods <-  c("Stan_models/Linear_model_RNaseP.stan")
...
write.csv(model_settings, "Outputs/model_settings_covariates.csv", row.names = F)
```
...
model_setup_f_name = paste0('Rout/model_settings_covariates_job_', i,'.RData')
save(model_settings, 
     adastra_dat_analysis,
     adastra_dat_i,
     stan_inputs, 
...
```{r}
for(i in 1:nrow(model_settings)){
  load(paste0('Rout/model_settings_covariates_job_', i,'.RData'))
...
file_name = paste0('Rout/model_fits_covariates_job_', i, '.RData') 
