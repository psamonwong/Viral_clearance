...
for(i in 1:length(ind)){
  ...
  posterior_params <- cbind(sigmasq_u1, sigmasq_u2, sigma_logvl, alpha_0, beta_0, sigmasq_theta1, sigmasq_theta2)
...
summary_Params$facet <- c("alpha[0]", "beta[0]", "sigma[logvl]", "sigma[theta[1]]^{2}", "sigma[theta[2]]^{2}", "sigma[u[1]]^{2}", "sigma[u[2]]^{2}")[as.numeric(summary_Params$params)]

```

```{r}
G <- ggplot(summary_Params %>% filter(!params %in% c('sigmasq_u1', 'sigmasq_u2'))
            , aes(x = Type, y = P50, col = Type)) +
  geom_point(size = 3) +
  geom_errorbar(aes(x = Type, ymin = P2.5, ymax = P97.5),
                width = 0.1, linewidth = 0.85) +
  theme_bw(base_size = 11) +
  facet_wrap(.~facet, scales = 'free', labeller = label_parsed, ncol = 3) +
