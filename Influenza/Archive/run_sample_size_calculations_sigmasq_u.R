...
k_sigmasq_u = c(0.5, 1, 2) # Varying baseline viral load and slope variations (inter-individual variations)
...
save(sim_settings, file = 'Rout/sim_settings_sigmasq_u.RData')
...
thetas$sigmasq_u[,1] <- thetas$sigmasq_u[,1] * sim_settings$k_sigmasq_u[i]
thetas$sigmasq_u[,2] <- thetas$sigmasq_u[,2] * sim_settings$k_sigmasq_u[i]
...
f_name = paste0('sims_out_sigmasq_u/sim_out_sigmasq_u_',i,'.csv')
print(f_name)
